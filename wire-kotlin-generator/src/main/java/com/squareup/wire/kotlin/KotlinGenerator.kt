...
class KotlinGenerator private constructor(
  ...
  private fun findFirstEnumValue(fieldType: ProtoType): String {
    return (schema.getType(fieldType) as EnumType).constants().first().name
  }
  ...
  private fun generateEnum(message: EnumType): TypeSpec {
    ...
    builder.addEnumConstant("UNRECOGNIZED", TypeSpec.anonymousClassBuilder()
  ...
  private fun Field.getDeclaration(allocatedName: String) = when {
    ...
    isMap -> CodeBlock.of("val $allocatedName = mutableMapOf<%T, %T>()", keyType.typeName, valueType.typeName)
    isEnum -> CodeBlock.of("var $allocatedName: %T = %T.%L", declarationClass, typeName , findFirstEnumValue(type()))
    else -> CodeBlock.of("var $allocatedName: %T = %L", declarationClass, getDefaultValue())
  }

  private val Field.declarationClass: TypeName
    get() = when {
      isEnum -> getClass().copy(nullable = false)
      isRepeated || default != null -> getClass()
      default == null -> {
        val cl = getClass()
        when (type().typeName.simpleName) {
          "String" -> cl.copy(nullable = false)
          "Long" -> cl.copy(nullable = false)
          "Int" -> cl.copy(nullable = false)
          "Float" -> cl.copy(nullable = false)
          "Double" -> cl.copy(nullable = false)
          "Boolean" -> cl.copy(nullable = false)
          else -> cl.copy(nullable = true)
        }
      }
      isMap -> getClass()
  ...
  private val Field.typeName: TypeName
    get() = when {
      ...
      isEnum -> type().typeName.copy(nullable = false)
      default != null -> type().typeName.copy(nullable = true)
      default == null -> {
        when (type().typeName.simpleName) {
          "String" -> type().typeName.copy(nullable = false)
          "Long" -> type().typeName.copy(nullable = false)
          "Int" -> type().typeName.copy(nullable = false)
          "Float" -> type().typeName.copy(nullable = false)
          "Double" -> type().typeName.copy(nullable = false)
          "Boolean" -> type().typeName.copy(nullable = false)
          else -> type().typeName.copy(nullable = true)
        }
      }
      else -> type().typeName
    }
